...
const NoteCard = ({ note }: NoteCardProps) => {
  return (
    <Dialog.Root>
      <Dialog.Trigger
        className='rounded-md text-left flex flex-col outline-none bg-slate-800 p-5 
        gap-3 overflow-hidden relative hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <div className='flex flex-1 flex-col gap-3 p-5'>
            <span className='text-sm font-medium text-slate-300'>
              {formatDistanceToNow(note.date.toISOString(), { locale: ptBR, addSuffix: true })}</span>
            <p className='text-slate-400 text-sm leading-6'>{note.content}</p>
          </div>
          <button
            type='button'
